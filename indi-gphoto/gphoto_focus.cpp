...
camera_auto_focus(Camera *camera, GPContext *context, int onoff) {
	...
	ret = _lookup_widget (widget, "autofocus", &child);
	if (ret < GP_OK) {
		fprintf (stderr, "lookup 'autofocus' failed: %d\n", ret);
		goto out;
	}
...
camera_manual_focus (Camera *camera, int xx, GPContext *context) {
	...
	ret = _lookup_widget (widget, "manualfocus", &child);
	if (ret < GP_OK) {
		fprintf (stderr, "lookup 'manualfocus' failed: %d\n", ret);
		goto out;
	}
